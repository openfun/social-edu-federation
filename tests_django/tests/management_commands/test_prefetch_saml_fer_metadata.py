...
def test_command_success(mocker, settings, verbosity):
    """Tests the `prefetch_saml_fer_metadata` works as expected in nominal case."""
    ...
    out = StringIO()
    call_command(
        "prefetch_saml_fer_metadata", ["saml_fer"], stdout=out, verbosity=verbosity
    )
...
def test_command_success_several_backends(mocker, settings):
    """Tests the management command allows several backends as an input."""
    settings.AUTHENTICATION_BACKENDS = (
        "social_edu_federation.backends.saml_fer.FERSAMLAuth",
    )
    settings.SOCIAL_AUTH_SAML_FER_FEDERATION_SAML_METADATA_STORE = (
...
def test_command_fails_base_metadata_store(settings):
    """
    The `prefetch_saml_fer_metadata` management command will fail when the metadata store
    associated to the backend, does not provide default cache methods.
    """
    settings.AUTHENTICATION_BACKENDS = (
        "social_edu_federation.backends.saml_fer.FERSAMLAuth",
    )
...
def test_command_fails_no_backend_provided(settings):
    """The `prefetch_saml_fer_metadata` management command will fail if no backend provided."""
    out = StringIO()
    err = StringIO()
    with pytest.raises(CommandError):
...
def test_command_fails_for_unknown_reason(mocker, settings):
    """
    The `prefetch_saml_fer_metadata` management command may fail for unknown reason.
    Among them is the HTTP request times out.
    This test asserts it raises a controlled error.
    """
    settings.AUTHENTICATION_BACKENDS = (
        "social_edu_federation.backends.saml_fer.FERSAMLAuth",
    )
